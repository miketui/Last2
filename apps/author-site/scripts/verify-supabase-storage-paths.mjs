...
export function verifyLockedPathStrings({ repoRoot = resolve(process.cwd(), "../.."), appDir = process.cwd() } = {}) {
  ...
  const docFiles = walk(resolve(repoRoot, "docs/website-v4")).filter((file) => /\.(?:md|sql|json)$/i.test(file));
  ...
  for (const file of files) {
    ...
    if (rel.startsWith("apps/author-site/public/") && (text.includes(lockedStorage.epub) || text.includes(lockedStorage.pdf))) {
