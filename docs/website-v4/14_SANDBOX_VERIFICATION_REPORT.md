...
cd apps/author-site && pnpm check:supabase-storage
cd apps/author-site && pnpm check:stripe-test
grep -RIn --exclude-dir=node_modules --exclude-dir=.next -E 'sk_live_|rk_live_|whsec_live|supabase_service_role=|STRIPE_SECRET_KEY=.+|RESEND_API_KEY=.+|MAILERLITE_API_KEY=.+|SUPABASE_SERVICE_ROLE_KEY=.+' apps/author-site docs/website-v4 .env.example || true
find apps/author-site/public -type f | grep -Ei '.(epub|pdf)$' && exit 1 || true
test ! -f apps/author-site/.env.local
test ! -f apps/author-site/.env.sandbox
```
...
| Private paid deliverables in public | PASSED | `pnpm check:deliverables` and `find apps/author-site/public ...` found no EPUB/PDF in public assets. |
...
| No committed local env files | PASSED | `test ! -f apps/author-site/.env.local` and `test ! -f apps/author-site/.env.sandbox` passed. |
...
- Paid EPUB/PDF release artifacts stayed under `release/` and outside `apps/author-site/public/`.
