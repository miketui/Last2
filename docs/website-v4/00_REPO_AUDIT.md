...
- `AGENTS.md` and `CODEX_MASTER_PROMPT.md` require an audit-first workflow, then a v4 spec package, then an app under `apps/author-site/` unless a newer authoritative app already exists.
...
- **New audit docs:** `docs/website-v4/` is now created only for this audit document.
...
- The safe future location remains `apps/author-site/`, but this audit phase intentionally does **not** create it.
...
|---|---|---|
| App location | References an existing `web/` site. | Create/update `apps/author-site/` unless a newer authoritative app already exists. None exists now. |
...
2. **Create the v4 spec package next** under `docs/website-v4/` before code:
...
5. **Only after specs are complete, scaffold `apps/author-site/`** with Next.js App Router, TypeScript strict, Tailwind, Supabase, Stripe, Resend, MailerLite, analytics, tests, and `.env.example` variable names only.
...
8. **Run validation from `apps/author-site/`** only after the app exists: `pnpm install`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`.
...
|---|---:|---|
| No web app exists yet | High | Expected for audit phase. Next code phase must scaffold `apps/author-site/` from scratch. |
...
```text
apps/author-site/package.json
apps/author-site/app/...
apps/author-site/components/...
apps/author-site/lib/...
apps/author-site/supabase/migrations/...
```
...
Prompt 2 should ask Codex to create the complete `docs/website-v4/` spec package listed above, using this audit as the baseline and reconciling the v3 marketing materials with the new locked Next.js + Supabase + Stripe architecture.

Recommended Prompt 2 scope:

- Write the v4 PRD, site tree/file map, ACISS token spec v2, funnel, page plans/wireframes, website copy v4, workflow/keys, build log, QA checklist, immersive style guide copy, and backend/analytics/subscription setup copy under `docs/website-v4/`.
- Keep the work documentation-only.
- Do not scaffold `apps/author-site/` yet.
