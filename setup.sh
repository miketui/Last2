...
if [[ -f pnpm-lock.yaml || -f package.json ]]; then
  ...
  warn "No package.json at repo root. Codex should scaffold the app under apps/author-site/."
fi
...
if [[ -d "$ROOT/apps/author-site" ]]; then
  log "Existing author site detected at apps/author-site; installing dependencies"
  cd "$ROOT/apps/author-site"
...
log "Setup summary"
{
  ...
  echo "Expected author site path: apps/author-site"
